...
if __name__ == "__main__":
    ...
    # Since SAC requires separate optimizers for actor and critic
    # we make a hack to combine them and create a single hivemind optimizer
    # (hack requires minor code change inside sb3)
    optimizer = torch.optim.Adam([
        *model.policy.actor.optimizer.param_groups,
        *model.policy.critic.optimizer.param_groups,
        # *model.ent_coef_optimizer.param_groups,
    ])
    hivemind_optimizer = hivemind.Optimizer(
        dht=dht,
        optimizer=optimizer,
        **configure_hivemind_opts(args),
    )
    model.policy.actor.optimizer = hivemind_optimizer
    model.policy.critic.optimizer = hivemind_optimizer
